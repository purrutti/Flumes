...
namespace SuperviFlume_v2
{
    ...
    public class WebSocketServer
    {
        ...
        /// <summary>Déclenché quand un aquarium reçoit de nouvelles données.</summary>
            ...
            try
            {
                ...
                switch (frame.cmd)
                {
                    ...
                    case 3: // SEND_DATA — données de mesure d'un aquarium
                        var aq = JsonConvert.DeserializeObject<Aquarium>(data);
                        aq.lastUpdated = DateTime.Now;
                        if (aq.ID >= 1 && aq.ID <= Aquariums.Count)
                        {
                            Aquariums[aq.ID - 1] = aq;
                            AquariumReceived?.Invoke(aq);
                        }
                        break;

                    case 6: // MASTER_DATA — circuit général (froid / chaud / ambiant)
                        var md = JsonConvert.DeserializeObject<MasterData>(data);
                        LastMasterData = md;
                        MasterDataReceived?.Invoke(md);
                        break;
